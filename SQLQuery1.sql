...
--order by cast- sorteerib päringutulemusi
select * from Person order by cast(Age as int)
select * from Person order by Age

--arvutab tabeli kõigi kirjete vanuste summa
...
---Iga tulemuse rida sisaldab unikaalset Name ja DepartmentId väärtuste paari.
select distinct Name, DepartmentId from Employees

---arvutab palgaväärtuste summa
select sum(cast(Salary as int)) from Employees
---arvutab miinimumpalga väärtuse kõigi tabeli kannete hulgast Employees
...
--protseduur võtab parameetri ja tagastab teatud soost töötajate arvu.
...
---arvutab tabelis olevate kirjete (töötajate) koguarvu
...
--- protseduur võimaldab saada töötaja nime tema tunnuse järgi
...
--Kui tabelis Töötajad on töötaja ID 6-ga, siis saadakse tulemuseks selle numbriga töötaja nimi
